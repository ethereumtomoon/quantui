...
let tx = new Point();
const benchmark=jsdt.data.result.benchmark
const gains=jsdt.data.result.gains
var v=[]
function tf(ls) {
    let ans=[]
    for (var i=0;i<ls.time.length;i++){
      var d = new Date();
      d.setTime(benchmark.time[i])
      var date0=d.toLocaleDateString()
      let n=tx.nowTime('yy-mm-dd',d)
      ans.push({"time":n,"value":ls.value[i]})
      //console.log(n,benchmark.value[i])
    }
    return ans
} 
let BM=tf(benchmark)
...
var overallreturnchart = createChart(document.body, {
  width: 800,
  height: 500,
  layout: {
    background: {
...
overallreturnlines.setData(tf(jsdt.data.result.overallReturn))
var gainschart = createChart(document.body, {
  width: 800,
  height: 500,
  layout: {
    background: {
...
var earnlines = gainschart.addLineSeries({
	color: 'rgba(0, 252, 0, 1)',
	lineWidth: 2,
}).setData(tf(jsdt.data.result.gains.earn))
var loselines = gainschart.addLineSeries({
	color: 'rgba(252, 0, 0, 1)',
	lineWidth: 2,
}).setData(tf(jsdt.data.result.gains.lose))
var orderschart = createChart(document.body, {
  width: 800,
  height: 500,
  layout: {
    background: {
...
var buylines = orderschart.addHistogramSeries({
	color: 'rgba(0, 252, 0, 1)',
	lineWidth: 2,
}).setData(tf(jsdt.data.result.orders.buy))
var selllines = orderschart.addHistogramSeries({
	color: 'rgba(252, 0, 0, 1)',
	lineWidth: 2,
}).setData(tf(jsdt.data.result.orders.sell))
console.log(BM)/**/
